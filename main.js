...
$(document).ready(function () {
  ...
  $("head").append(
    "<style type='text/css'>body { background: #3f51b5; }</style>"
  );
...
//   console.log(chrome.serial);
   ...
   navigator.serial.addEventListener("disconnect", (e) => {
	...
	navigator.serial.getPorts().then((ports) => {
		...
		ports.forEach((port) => {
		    ...
		    console.log("USB Vendor ID: " + port.usbVendorId);
		    console.log("USB Product ID: " + port.usbProductId);
		    console.log("-----------------------------------");
		});
	 });
  ...
  $("#send_button").bind("click", function (event, ui) {
    if (connected) {
      ...
      chrome.serial.send(
        connectionId,
        convertStringToArrayBuffer(textToSend),
        function (sendInfo) {
          if (sendInfo.error)
            $.modal(
              '<div id="title">Unable to send data: ' +
                sendInfo.error +
                "</div>"
            );
        }
...
function onConnect(connectionInfo) {
  ...
  if (connectionInfo) {
    ...
    else errorMsg = "Failed to connect to the port.";
